...
@Service
public class CikeReturnRechargeServiceImpl implements CikeReturnRechargeService {

	private static final Logger LOGGER = LoggerFactory.getLogger(CikeReturnRechargeServiceImpl.class);
	@Autowired
	...
	@Override
	@Transactional
	public void cikeAsynchronousResultRecharge(Map<String, Object> params) {
//		Map<String,String> params = new HashMap<>();
		int i = 0;
		try {
//			params = RequestMap.getParameterMap(request);
			LOGGER.info("金顺返回参数信息" + params.toString());
			if("SUCCESS".equals(params.get("tradeStatus"))) {
				//验签
				Map<String, Object> signMap = new HashMap<>();
				signMap.put("merId", params.get("merId"));
				signMap.put("merOrderId", params.get("merOrderId"));
				signMap.put("tranAmt", params.get("tranAmt"));
				signMap.put("paySuccessTime", params.get("paySuccessTime"));
				signMap.put("signType", params.get("signType"));
				String sign = SignUtil.getSignToCiKePayReturn(signMap, cikeProperties.getMd5key());
				if(sign.equals(params.get("sign"))) {
					//查询订单
					Map<String, Object> order = cikeMapper.getCikeTradeByOrderNo(StringUtil.getMapValue(params, "merOrderId"));
					...
					edit_user.put("balance_num", params.get("tranAmt"));//支付余额账户
					...
					edit_order.put("pay_datetime", params.get("paySuccessTime"));
					edit_order.put("up_date", date);//更新日期
					edit_order.put("up_time", time);//更新时间
					i = cikeMapper.updateOrderStatus(edit_order);
					if(i != 1){
					...
					i = cikeMapper.updateRechargeOrderStatus(edit_recharge);
					if(i != 1){
						throw new Exception("订单状态更新异常");
					}
				}else {
			...
			LOGGER.error("CikeReturnRechargeServiceImpl -- cikeAsynchronousResultRecharge方法执行异常:"+ExceptionUtil.getExceptionAllinformation(e));
		}
	}
